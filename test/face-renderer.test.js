...
test('ガラスの縁を顔サイズに合わせて細く保つ', () => {
    ...
    assert.equal(FaceRenderer.calculateGlassRim(400, 480).lineWidth, 3);
});
...
test('算出済みの顔領域を円形グラデーション付きで描画する', () => {
    ...
    assert.deepEqual(
        ...
        [
            ...
            ['color', 0.42, 'rgba(236, 244, 248, 0.025)'],
            ['color', 0.72, 'rgba(246, 250, 252, 0.065)'],
            ['color', 0.9, 'rgba(255, 255, 255, 0.085)'],
            ['color', 0.98, 'rgba(255, 255, 255, 0)'],
            ['color', 0, 'rgba(255, 255, 255, 0.26)'],
            ['color', 0.35, 'rgba(248, 251, 253, 0.12)'],
            ['color', 0.7, 'rgba(196, 208, 218, 0.1)'],
            ['color', 1, 'rgba(255, 255, 255, 0.22)']
        ]
    );
    const gradients = target.calls.filter(call => call[0] === 'gradient');
    assert.deepEqual(gradients[1], ['gradient', 36, 44, 22, 40, 48, 40]);
