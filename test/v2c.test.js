...
/**
 * Node.jsが読み取り専用で提供するグローバル値も、テスト中だけ安全に差し替える。
 */
const replaceGlobalForTest = (t, name, value) => {
    const originalDescriptor = Object.getOwnPropertyDescriptor(global, name);
    ...
    t.after(() => {
        if (originalDescriptor) {
            Object.defineProperty(global, name, originalDescriptor);
            return;
        }
...
test('カメラAPI未対応時は同期例外にせずエラー処理へ渡す', async t => {
    ...
    replaceGlobalForTest(t, 'navigator', {});
