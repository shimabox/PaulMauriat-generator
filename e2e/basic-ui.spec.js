...
test.beforeEach(async ({ page }) => {
    await page.addInitScript(() => {
        ...
        window.__faceTrackerMock = {
            enabled: false,
            positions: false
        };

        // generatorの初期化前に顔追跡実装を差し替え、検出状態をテストから制御する。
...
test('顔検出後はPNGをダウンロードできる', async ({ page }) => {
    const fixturePath = path.join(__dirname, 'fixtures', 'background.svg');
    await page.evaluate(() => { window.__cameraMock.mode = 'success'; });
    await setFaceDetected(page, true);
    await page.locator('#read-file').setInputFiles(fixturePath);
...
test('対応端末では生成画像とハッシュタグを共有できる', async ({ page }) => {
    const fixturePath = path.join(__dirname, 'fixtures', 'background.svg');
    await page.evaluate(() => {
        window.__cameraMock.mode = 'success';
    ...
    const shareButton = page.getByRole('button', { name: 'Share generated image' });
    await page.getByRole('button', { name: 'Stop face tracking' }).click();
    await expect(shareButton).toBeEnabled();
    await shareButton.click();

    await expect(page.locator('#status-message')).toBeEmpty();
    expect(await page.evaluate(() => window.__sharedImage)).toEqual({
...
test('ファイル共有に未対応なら画像を保存してXの投稿画面を開く', async ({ page }) => {
    const fixturePath = path.join(__dirname, 'fixtures', 'background.svg');
    await page.evaluate(() => {
        window.__cameraMock.mode = 'success';
        window.__sharedDownload = null;
        window.__openedShareWindow = null;
        Object.defineProperty(navigator, 'canShare', {
            configurable: true,
            value: () => false
        });
        HTMLAnchorElement.prototype.click = function() {
            window.__sharedDownload = {
                filename: this.download,
                href: this.href
            };
        };
        window.open = (...args) => {
            window.__openedShareWindow = args;
            return {};
        };
    });
    await setFaceDetected(page, true);
    ...
    await expect(page.locator('#status-message')).toHaveText(
        '画像を保存しました。Xの投稿画面で画像を添付してください'
    );
    const download = await page.evaluate(() => window.__sharedDownload);
    expect(download.filename).toBe('paulmauriat.png');
    expect(download.href).toMatch(/^data:image\/png;base64,/);
    const openedWindow = await page.evaluate(() => window.__openedShareWindow);
    expect(openedWindow.slice(1)).toEqual(['_blank', 'noopener,noreferrer']);
    expect(decodeURIComponent(openedWindow[0])).toBe(
        'https://x.com/intent/post?text=#ポールモーリアジェネレーター'
    );
});
...
test('停止中かつ顔検出済みの場合だけ保存できる', async ({ page }) => {
    const fixturePath = path.join(__dirname, 'fixtures', 'background.svg');
    const captureButton = page.getByRole('button', { name: 'Download generated image' });
    const shareButton = page.getByRole('button', { name: 'Share generated image' });
    await page.evaluate(() => { window.__cameraMock.mode = 'success'; });
    ...
    await setFaceDetected(page, false);
    await page.getByRole('button', { name: 'Start face tracking' }).click();
    await expect(captureButton).toBeDisabled();
    await expect(shareButton).toBeDisabled();
});
